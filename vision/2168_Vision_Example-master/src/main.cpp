...
//#define Y_IMAGE_RES 756
#define Y_IMAGE_RES 5711.7
#define VIEW_ANGLE 35
...
//Target Ratio Ranges
double MinHRatio = 0.3;
double MaxHRatio = 3.0;

double MinVRatio = 1.5;
double MaxVRatio = 5.0;
double center = 275;
int MAX_SIZE = 255;
...
int main(int argc, const char* argv[])
{
	...
	progRun = false;
	//table = NetworkTable::GetTable("Data Table");
		...
		if (params.Process && progRun)
		{
			...
			if (!frame.empty())
			{
				...
				if(params.Debug)
				{
					...
					//cout<<"Contour0:" <<contour0<<endl;
					//cout<<"Contour1:" <<contour1<<endl;
					//table -> PutNumber("Distance to Target:", targets.targetDistance);
					//table -> PutNumber("Straight Distance:", straightDistance);
					cout<<"StraightDistance:" <<straightDistance<<endl;
					if((((contour1 - contour0)/2)+contour0-center) < 3 && (((contour1 - 							contour0)/2)+contour0-center) > -3) {
						if ((WHRatio0+WHRatio1)/2 < 0.55 && (WHRatio0+WHRatio1)/2 > 0.45) {
...
void CalculateDist(Target& targets)
{
	...
	double constant = 3674.387;
	
	//1. check if its in the centre of frame
	//2. make it centre of frame
	//3. check if ratio between width and height is the same as the normal ratio
	//4. make it the same ratio
	//5. move straight. 
	/*if (right side>left side){
		//move robot to left and then rotate clockwise;	
	}
		if (b>=10) {
			//move left;
		}
	...
	//targets.targetDistance = (Y_IMAGE_RES * targetHeight) / (height * 24 * tan((VIEW_ANGLE * PI) / (180 * 2)));
	targets.targetDistance = 0.0063 * ((height0+height1)/2) * ((height0+height1)/2) - 1.728 * ((height0+height1)/2) + 145.16;
	straightDistance = targets.targetDistance * (1.2881*(WHRatio0/WHRatio1) - 0.2716);
...
void findTarget(Mat original, Mat thresholded, Target& targets, const ProgParams& params)
{
	...
	if (!contours.empty() && !hierarchy.empty())
	{
		...
		for (unsigned int i = 0; i < contours.size(); i++)
		{
			...
			//int contourTotal = box. 
			//if(3+((contour1 - contour0)/2)+contour0-center
			//if(box.x < centre && contourTotal < box.x/2) {
			//	contourTotal += box.x;
			//}
			//else if (box.x > centre && contourTotal < box.x/2) {
			//	contourTotal += box.x;			
			//}
			if (i == 0) {
				contour0 = box.x;
...
Mat ThresholdImage(Mat original)
{
  ...
  inRange(imgHSV, Scalar(59,60,61), Scalar(93,255,255), imgThresholded); //Threshold the image   
